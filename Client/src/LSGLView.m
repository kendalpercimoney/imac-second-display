...
- (void)setCursorX:(int)x y:(int)y visible:(BOOL)visible {
    ...
    if (changed && _contextReady && [self window]) [self renderNow];
}
...
- (void)renderNow {
    ...
    if (_currentBuffer) {
        ...
        [self drawCursorIntoViewWidth:viewWidth height:viewHeight];
    }
...
- (void)drawCursorIntoViewWidth:(GLsizei)viewWidth height:(GLsizei)viewHeight {
    if (_pendingCursorImage) {
        ...
        glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA,
                     _cursorImageWidth, _cursorImageHeight, 0,
                     GL_RGBA, GL_UNSIGNED_BYTE, [_pendingCursorImage bytes]);
        _pendingCursorImage = nil;
        _haveCursorImage = YES;
    }
    if (!_haveCursorImage || !_cursorVisible || !_currentBuffer) return;

    IOSurfaceRef surface = CVPixelBufferGetIOSurface(_currentBuffer);
    if (!surface) return;
    ...
    double left = originX + ((double)_cursorX - _cursorHotspotX) * scale;
    double top = originY + drawHeight - ((double)_cursorY - _cursorHotspotY) * scale;
    double w = _cursorImageWidth * scale, h = _cursorImageHeight * scale;
    ...
    glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
    glBegin(GL_QUADS);
        glTexCoord2f(0.0f, (GLfloat)_cursorImageHeight); glVertex2d(left, top - h);
        glTexCoord2f((GLfloat)_cursorImageWidth, (GLfloat)_cursorImageHeight); glVertex2d(left + w, top - h);
        glTexCoord2f((GLfloat)_cursorImageWidth, 0.0f); glVertex2d(left + w, top);
